import {
  ...
  ValidationError,
  helpers
} from "./chunk-HXTE4TJ6.js";
import {
  fields,
  messages
} from "./chunk-KC6PIXYO.js";
import {
  __export
...
var requiredWhen = createRule(
  function requiredWhen2(_, checker, field) {
...
// src/schema/base/conditional_rules.ts
...
// src/schema/base/literal.ts
var NullableModifier = class _NullableModifier {
  #parent;
  constructor(parent) {
    this.#parent = parent;
  }
...
var OptionalModifier = class _OptionalModifier extends ConditionalValidations {
...
var TransformModifier = class _TransformModifier {
...
var enumRule = createRule(function enumList(value, options, field) {
...
var dateRule = createRule(
  function date(value, options, field) {
    if (!field.isDefined) {
      return false;
    }
...
var equalsRule = createRule(function equals(_, options, field) {
...
var afterRule = createRule(function after(_, options, field) {
  const compare = options.compare || "day";
  const format = options.format || field.$formats;
  const dateTime = field.$value;
  const expectedValue = typeof options.expectedValue === "function" ? options.expectedValue(field) : options.expectedValue;
...
var afterOrEqualRule = createRule(function afterOrEqual(_, options, field) {
  const compare = options.compare || "day";
  const format = options.format || field.$formats;
  const dateTime = field.$value;
  const expectedValue = typeof options.expectedValue === "function" ? options.expectedValue(field) : options.expectedValue;
...
var beforeRule = createRule(function before(_, options, field) {
  const compare = options.compare || "day";
  const format = options.format || field.$formats;
  const dateTime = field.$value;
  const expectedValue = typeof options.expectedValue === "function" ? options.expectedValue(field) : options.expectedValue;
...
var beforeOrEqualRule = createRule(function beforeOrEqual(_, options, field) {
  const compare = options.compare || "day";
  const format = options.format || field.$formats;
  const dateTime = field.$value;
  const expectedValue = typeof options.expectedValue === "function" ? options.expectedValue(field) : options.expectedValue;
...
var sameAsRule = createRule(function sameAs(_, options, field) {
  const compare = options.compare || "day";
  const dateTime = field.$value;
  const format = options.format || field.$formats;
...
var notSameAsRule = createRule(function notSameAs(_, options, field) {
  const compare = options.compare || "day";
  const dateTime = field.$value;
  const format = options.format || field.$formats;
  const expectedValue = helpers.getNestedValue(options.otherField, field);
...
var afterFieldRule = createRule(function afterField(_, options, field) {
  const compare = options.compare || "day";
  const dateTime = field.$value;
  const format = options.format || field.$formats;
  const expectedValue = helpers.getNestedValue(options.otherField, field);
...
var afterOrSameAsRule = createRule(function afterOrSameAs(_, options, field) {
  const compare = options.compare || "day";
  const dateTime = field.$value;
  const format = options.format || field.$formats;
  const expectedValue = helpers.getNestedValue(options.otherField, field);
...
var beforeFieldRule = createRule(function beforeField(_, options, field) {
  const compare = options.compare || "day";
  const dateTime = field.$value;
  const format = options.format || field.$formats;
  const expectedValue = helpers.getNestedValue(options.otherField, field);
...
var beforeOrSameAsRule = createRule(function beforeOrSameAs(_, options, field) {
  const compare = options.compare || "day";
  const dateTime = field.$value;
  const format = options.format || field.$formats;
  const expectedValue = helpers.getNestedValue(options.otherField, field);
...
var weekendRule = createRule(function weekend(_, __, field) {
  const dateTime = field.$value;
...
var weekdayRule = createRule(function weekday(_, __, field) {
  const dateTime = field.$value;
  const day = dateTime.day();
  if (day === 0 || day === 6) {
...
// src/schema/null/main.ts
import camelcase2 from "camelcase";
var VineNull = class _VineNull {
  /**
   * The subtype of the literal schema field
   */
  [SUBTYPE] = "null";
   ...
   */
  clone() {
    return new _VineNull(this.cloneOptions());
  }
  /**
...
// src/schema/optional/main.ts
import camelcase3 from "camelcase";
var VineOptional = class _VineOptional extends ConditionalValidations {
  /**
   * The subtype of the literal schema field
   */
  [SUBTYPE] = "optional";
   ...
   */
  clone() {
    return new _VineOptional(this.cloneOptions(), this.cloneValidations());
  }
  /**
   * Mark the field under validation to be null. The null value will
   * be written to the output as well.
...
var NullableModifier2 = class _NullableModifier {
  /**
   * Reference to the parent schema being modified
   */
  #parent;
...
var OptionalModifier2 = class _OptionalModifier extends ConditionalValidations {
  /**
   * Reference to the parent schema being modified
   */
  #parent;
...
var BaseType = class extends Macroable2 {
  /**
...
var minLengthRule = createRule(function minLength(value, options, field) {
  if (value.length < options.min) {
    field.report(messages["array.minLength"], "array.minLength", field, options);
  }
});
var maxLengthRule = createRule(function maxLength(value, options, field) {
  if (value.length > options.max) {
    field.report(messages["array.maxLength"], "array.maxLength", field, options);
  }
});
var fixedLengthRule = createRule(
...
var notEmptyRule = createRule(function notEmpty(value, _, field) {
  if (value.length <= 0) {
    field.report(messages.notEmpty, "notEmpty", field);
  }
});
var distinctRule = createRule(
...
var compactRule = createRule(function compact(value, _, field) {
  if (!field.isValid) {
    return;
  }
  field.mutate(
...
var VineCamelCaseObject = class _VineCamelCaseObject extends BaseType {
  /**
...
var VineObject = class _VineObject extends BaseType {
  ...
  [PARSE](propertyName, refs, options) {
    return {
      type: "object",
      fieldName: propertyName,
      propertyName: options.toCamelCase ? camelcase7(propertyName) : propertyName,
      bail: this.options.bail,
      allowNull: this.options.allowNull,
      isOptional: this.options.isOptional,
      parseFnId: this.options.parse ? refs.trackParser(this.options.parse) : void 0,
...
var mobileRule = createRule(function mobile(value, options, field) {
...
var regexRule = createRule(function regex(value, expression, field) {
  if (!expression.test(value)) {
    field.report(messages.regex, "regex", field);
  }
});
var hexCodeRule = createRule(function hexCode(value, _, field) {
  if (!helpers.isHexColor(value)) {
    field.report(messages.hexCode, "hexCode", field);
  }
});
var urlRule = createRule(function url(value, options, field) {
  if (!helpers.isURL(value, options)) {
    field.report(messages.url, "url", field);
  }
});
var activeUrlRule = createRule(async function activeUrl(value, _, field) {
  if (!await helpers.isActiveURL(value)) {
    field.report(messages.activeUrl, "activeUrl", field);
  }
});
var alphaRule = createRule(
...
var minLengthRule3 = createRule(function minLength3(value, options, field) {
  if (value.length < options.min) {
    field.report(messages.minLength, "minLength", field, options);
  }
});
var maxLengthRule3 = createRule(function maxLength3(value, options, field) {
  if (value.length > options.max) {
    field.report(messages.maxLength, "maxLength", field, options);
  }
});
var fixedLengthRule3 = createRule(
  function fixedLength3(value, options, field) {
    if (value.length !== options.size) {
      field.report(messages.fixedLength, "fixedLength", field, options);
    }
  }
);
var endsWithRule = createRule(
...
var sameAsRule2 = createRule(
  function sameAs2(value, options, field) {
...
var notSameAsRule2 = createRule(
  function notSameAs2(value, options, field) {
    const input = helpers.getNestedValue(options.otherField, field);
    if (input === value) {
...
var confirmedRule = createRule(function confirmed(value, options, field) {
...
var creditCardRule = createRule(function creditCard(value, options, field) {
...
var passportRule = createRule(function passport(value, options, field) {
...
var postalCodeRule = createRule(function postalCode(value, options, field) {
...
var ulidRule = createRule(function ulid(value, _, field) {
  if (!helpers.isULID(value)) {
    field.report(messages.ulid, "ulid", field);
  }
});
var asciiRule = createRule(function ascii(value, _, field) {
  if (!helpers.isAscii(value)) {
    field.report(messages.ascii, "ascii", field);
  }
});
var ibanRule = createRule(function iban(value, _, field) {
  if (!helpers.isIBAN(value)) {
    field.report(messages.iban, "iban", field);
  }
});
var jwtRule = createRule(function jwt(value, _, field) {
  if (!helpers.isJWT(value)) {
    field.report(messages.jwt, "jwt", field);
  }
});
var coordinatesRule = createRule(function coordinates(value, _, field) {
  if (!helpers.isLatLong(value)) {
    field.report(messages.coordinates, "coordinates", field);
  }
});
var trimRule = createRule(function trim(value, _, field) {
  if (!field.isValid) {
    return;
  }
...
var toCamelCaseRule = createRule(function toCamelCase(value, _, field) {
  if (!field.isValid) {
    return;
  }
  field.mutate(camelcase9(value), field);
});
var escapeRule = createRule(function escape(value, _, field) {
  if (!field.isValid) {
    return;
  }
...
var numberRule = createRule(function number(value, options, field) {
  if (!field.isDefined) {
    return false;
  }
...
var minRule = createRule(function min(value, options, field) {
  if (value < options.min) {
    field.report(messages.min, "min", field, options);
  }
});
var maxRule = createRule(function max(value, options, field) {
  if (value > options.max) {
    field.report(messages.max, "max", field, options);
  }
});
var rangeRule = createRule(
...
var positiveRule = createRule(function positive(value, _, field) {
  if (value > 0) {
    return;
  }
  field.report(messages.positive, "positive", field);
});
var negativeRule = createRule(function negative(value, _, field) {
  if (value < 0) {
    return;
  }
  field.report(messages.negative, "negative", field);
});
var nonNegativeRule = createRule(function nonNegative(value, _, field) {
  if (value >= 0) {
    return;
  }
  field.report(messages.nonNegative, "nonNegative", field);
});
var nonPositiveRule = createRule(function nonPositive(value, _, field) {
  if (value <= 0) {
    return;
  }
  field.report(messages.nonPositive, "nonPositive", field);
});
var decimalRule = createRule(
...
var withoutDecimalsRule = createRule(function withoutDecimals(value, _, field) {
  if (!Number.isInteger(value)) {
    field.report(messages.withoutDecimals, "withoutDecimals", field);
  }
});
var inRule2 = createRule(function inValues(value, options, field) {
...
var VineNumber = class _VineNumber extends BaseLiteralType {
   ...
   */
  decimal(range2) {
    return this.use(decimalRule({ range: Array.isArray(range2) ? range2 : [range2] }));
  }
  /**
...
var equalsRule2 = createRule(
  function equals2(value, options, field) {
...
var acceptedRule = createRule(function accepted(value, _, field) {
...
// src/schema/native_file/rules.ts
var isNativeFileRule = createRule(function file(value, _, field) {
  if (!field.isDefined) {
    return false;
  }
...
var minSizeRule = createRule(function minSize(value, options, field) {
...
var maxSizeRule = createRule(function minSize2(value, options, field) {
  if (value.size > options.max) {
    field.report(messages["nativeFile.maxSize"], "nativeFile.maxSize", field, options);
  }
});
var mimeTypesRule = createRule((value, options, field) => {
...
// src/schema/native_file/main.ts
var VineNativeFile = class _VineNativeFile extends BaseLiteralType {
  [SUBTYPE] = "file";
  constructor(options, validations) {
    super(options, validations || []);
  ...
  clone() {
    return new _VineNativeFile(this.cloneOptions(), this.cloneValidations());
  }
  /**
...
import camelcase10 from "camelcase";
var VineUnionOfTypes = class _VineUnionOfTypes {
   ...
   */
  clone() {
    const cloned = new _VineUnionOfTypes(this.#schemas.map((schema) => schema.clone()));
    cloned.otherwise(this.#otherwiseCallback);
    return cloned;
  }
  /**
   ...
   */
  nullable() {
    return new _VineUnionOfTypes([new VineNull(), ...this.#schemas]);
  }
  /**
   * Compiles to a union
   */
  [PARSE](propertyName, refs, options) {
    return {
      type: "union",
      fieldName: propertyName,
      propertyName: options.toCamelCase ? camelcase10(propertyName) : propertyName,
      elseConditionalFnRefId: refs.trackConditional(this.#otherwiseCallback),
...
var VineValidator = class {
  ...
  "~standard" = {
    ...
    validate: async (data) => {
      ...
      return {
        issues: error?.messages.map((message) => {
          return {
            ...message,
            path: message.field
          };
